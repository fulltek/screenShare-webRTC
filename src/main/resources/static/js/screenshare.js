...
//	setTimeout(function(){console.log(peerConnection)},1000);
    /*
...
async function displayLocalStreamAndSignal(firstTime) {
    ...
    try {
        ...
        console.log(new Date(),'Received local stream');
    ...
    console.log(new Date(),'Start complete');
};
...
function logVideoAudioTrackInfo(localStream) {
    ...
    if (videoTracks.length > 0) {
        console.log(new Date(),`Using video device: ${videoTracks[0].label}`);
    }
    if (audioTracks.length > 0) {
        console.log(new Date(),`Using audio device: ${audioTracks[0].label}`);
    }
};